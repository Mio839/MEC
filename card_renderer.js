        ...
        '<div class="qb">' +
          ...
          '<div class="ab">' +
            ...
            '<div>' +
              ...
              // ans_sub は qt / eg[].c と同じく生HTML。kw3（正解の核）・kw4（誤りの理由）・
              // sub/sup（SpO2 等）を書けないと、執筆時に入れた強調がそのままタグの文字列として
              // 画面に出る（m121s 65問・resp 152問が実際にそうなっていた）。
              // ⚠️ ans_label（.ac）はエスケープのまま——計算問題の正解「計算答：<桁文字列>」を
              // 掴む正本で、採点が文字列一致に依存している。
              // ⚠️ 全8328問を検査したうえでの変更: ans_sub の `<` は m121s/resp の意図した
              // タグ以外すべて数値比較（`<60/分` `<20mEq/L`）で、HTML は `<` の直後が英字の
              // ときしかタグと解釈しないため素の文字として出る。HTML実体（&…;）も0件。
              // 新しく数値比較を書くときも `<` の直後に英字を置かないこと。
              '<div class="as">' + q.ans_sub + '</div>' +
            '</div>' +
          '</div>' +
