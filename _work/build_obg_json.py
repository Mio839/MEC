...
build_peds_json.py と同構造（小児科と同じく data-uid / data-rate が無い形式）:
  ...
  - rate は cr スパンの "98%" から解析
  - 各ファイル=1章、章名は obg_questions.json から取得
  - 画像 src は "images/xxx" → "産婦人科/images/xxx"（study.htmlはルート基準）
questions_obg.js ラッパは pre-commit フック（gen_js_from_json.js）が自動生成する。
"""
...
with open(OBG_DIR / 'obg_questions.json', encoding='utf-8') as f:
    obg_meta = json.load(f)
chap_names = {c['chapter']['num']: c['chapter']['name'] for c in obg_meta}

chapters = []
total = n_img = 0
for hf in sorted(glob.glob(str(OBG_DIR / 'ch*.html'))):
