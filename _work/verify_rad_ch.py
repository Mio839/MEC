# -*- coding: utf-8 -*-
"""
questions_rad.json を新科目HTML生成ガイド §2（品質基準）・§4（採点/画像の不変条件）で検査する。
`_work/verify_anes_ch.py` の放射線科版。

  python _work/verify_rad_ch.py            全章
  python _work/verify_rad_ch.py --ch 1     指定章のみ

さらに巻末解答一覧表（_work/_rad_tmp/anstable.json）と
 正解ラベル・正答率・バッジ・問題数 を突合する（PDFが正本）。
"""
import argparse, io, json, os, re, sys

BASE = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
JSON = os.path.join(BASE, 'questions_rad.json')
ANS = os.path.join(BASE, '_work', '_rad_tmp', 'anstable.json')
...
def main():
    ...
    if not os.path.exists(ANS):
        ...
        print('%s が無い。先に `python _work/rad_pdf.py anstable` を実行すること。' % ANS)
        return 1
