# -*- coding: utf-8 -*-
"""
放射線科の章別HTML（放射線科/chNN_*.html）から questions_rad.json を生成する。
build_anes_json.py と同構造:
  - uid = rad_ch{nn}_{card_id}（例 rad_ch01_q1）
  - rate は cr スパンの "45%" から解析
  - 各ファイル=1章、章名は rad_questions.json から取得
  - 画像 src は "images/xxx" → "放射線科/images/xxx"（study.htmlはルート基準）
"""
...
with open(RAD_DIR / 'rad_questions.json', encoding='utf-8') as f:
    rad_meta = json.load(f)
chap_names = {c['chapter']['num']: c['chapter']['name'] for c in rad_meta}

chapters = []
total = n_img = 0
for hf in sorted(glob.glob(str(RAD_DIR / 'ch*.html'))):
