# -*- coding: utf-8 -*-
"""
呼吸器PDF（MEC臓器別講座・呼吸器・341ページ）から設問の原文・画像を取り出し、
questions_resp.json と突き合わせる。circ_pdf.py の同型（版面が同じ臓器別講座なので
解答一覧表の列座標まで一致する: 出題テーマ@263.7 / 疾患名@405.4 / 必修○@477 / 一般○@494 / 臨床○@511）。

    python _work/resp_pdf.py pages                  各章がPDFの何ページかを推定
    python _work/resp_pdf.py text   --ch 1          その章の設問原文をdump（Readで読む用）
    python _work/resp_pdf.py qtdiff --ch 1          JSONのqtとPDF原文の乖離を検出
    python _work/resp_pdf.py images --ch 1          画像矩形＋帰属候補＋ページ描画を出す
    python _work/resp_pdf.py save   --ch 1          ch01_map.txt を検収してから保存
...
PDF = os.path.join(BASE, 'MEC問題文pdf', 'MEC臓器別講座・呼吸器_問題（表紙2026）.pdf')
JSON_PATH = os.path.join(BASE, 'questions_resp.json')
OUT = os.path.join(BASE, '_work', '_resp_tmp')
DEST_IMG = os.path.join(BASE, '呼吸器', 'images')
