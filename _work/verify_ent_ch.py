# -*- coding: utf-8 -*-
"""
questions_ent.json を新科目HTML生成ガイド §2（品質基準）・§4（採点/画像の不変条件）で検査する。
`_work/verify_oph_ch.py` の耳鼻咽喉科版。

  python _work/verify_ent_ch.py            全章
  python _work/verify_ent_ch.py --ch 1     指定章のみ

さらに巻末解答一覧表（_work/_ent_tmp/anstable.json）と
 正解ラベル・正答率・バッジ・問題数 を突合する（PDFが正本）。
"""
import argparse, io, json, os, re, sys

BASE = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
JSON = os.path.join(BASE, 'questions_ent.json')
ANS = os.path.join(BASE, '_work', '_ent_tmp', 'anstable.json')
...
def main():
    ...
    if not os.path.exists(ANS):
        # _work/_ent_tmp/ は .gitignore 済み＝別マシンでは存在しない。作り直す。
        print('%s が無い。先に `python _work/ent_pdf.py anstable` を実行すること。' % ANS)
        return 1
