# -*- coding: utf-8 -*-
"""
泌尿器科PDF（MECマイナー講座・泌尿器科）から章を作るための抽出ツール。
`_work/ent_pdf.py` の座標系をそのまま流用（解答一覧表の列xは精神科・皮膚科・眼科・耳鼻咽喉科と同一）。

  python _work/uro_pdf.py anstable          巻末解答一覧表(p155-159)を全問ぶん抽出
  python _work/uro_pdf.py spec   --ch 1     指定章の NO/国試番号/正解/バッジ/正答率 一覧
  python _work/uro_pdf.py text   --ch 1     指定章の問題ページ本文を dump（Readで読む用）
  python _work/uro_pdf.py images --ch 1     画像の矩形と設問への帰属候補＋ページ画像（目視確認用）
  python _work/uro_pdf.py save   --ch 1     images が出した map ファイルに従って画像を保存

出力先は既定で _work/_uro_tmp/。Bashの標準出力はcp932で潰れるので、
生成された .txt を Read ツールで読むこと。

⚠️ PDF p.4 は「MECマイナー講座の特徴」の見本ページで、載っている問題は精神科のもの。
   泌尿器科の問題として拾わないこと（本ツールは章ページ範囲で限定しているので混入しない）。
"""
...
PDF = os.path.join(BASE, 'MEC問題文pdf', 'MECマイナー講座・泌尿器科_問題（表紙2026）.pdf')
OUT = os.path.join(BASE, '_work', '_uro_tmp')
DEST_IMG = os.path.join(BASE, '泌尿器科', 'images')

TABLE_PAGES = range(155, 160)          # 巻末「解 答」一覧表
INDEX_PAGES = range(160, 162)          # 巻末「国試番号索引」
...
CH = {
    1: (1, 44, 5, 22, '泌尿器の基本'),
    2: (45, 69, 23, 40, '腎'),
    3: (70, 133, 41, 86, '尿管・膀胱・尿道'),
    4: (134, 186, 87, 116, '前立腺'),
    5: (187, 214, 117, 136, '精巣・外性器'),
    6: (215, 242, 137, 154, '泌尿器科感染症'),
}

# 解答一覧表の列の左端x（精神科・皮膚科・眼科・耳鼻咽喉科と同一）
...
def cmd_anstable(_args):
    ...
    ⚠️ 章区切り行（「　2　〔 問題 〕腎」）が NO列・解答列に食い込むため、
