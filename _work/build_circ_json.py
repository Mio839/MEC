# -*- coding: utf-8 -*-
"""
循環器の章別HTML（循環器/chNN_*.html）から questions_circ.json を生成する。
build_oph_json.py / build_derm_json.py と同構造:
  - uid = circ_ch{nn}_{card_id}（例 circ_ch07_q442）
  - rate は cr スパンの "正答率 85%" から解析（.cr が無ければ rate=-1・rate_text=''）
  - 各ファイル=1章、章名は 循環器/circ_questions.json から取得
  - 画像 src は "images/xxx" → "循環器/images/xxx"（study.htmlはルート基準）

⚠️ 循環器は「新科目」ではなく既存科目の作り直しなので、uid が1つでも変わると
   done_v2 / mec_srs_v1 / myrate_v1 / mec_attempts_v1 の学習履歴が失われる。
   カードの id は必ず現行 questions_circ.json の uid 末尾と一致させること
   （_work/scaffold_circ_ch.py が自動転記するので手書きしない）。
   検証は python _work/roundtrip_circ.py。
"""
...
def main():
    with open(SUBJ_DIR / 'circ_questions.json', encoding='utf-8') as f:
        meta = json.load(f)
    chap_names = {c['chapter']['num']: c['chapter']['name'] for c in meta}

    chapters = []
    total = n_img = 0
    for hf in sorted(glob.glob(str(SUBJ_DIR / 'ch*.html'))):
        ...
        # ⚠️ 章番号と章名の区切りは U+2003（EM SPACE）。既存 questions_circ.json がこれで、
        #    半角スペースにすると章タイトルが全問ぶん差分になる（眼科は半角だが循環器は違う）。
        title = f'第{ch_num}章 {chap_names.get(ch_num, "")}'.strip()
        chapters.append({'title': title, 'qs': qs})
        total += len(qs)
        ni = sum(1 for q in qs if q['imgs'])
        n_img += ni
        print(f'  {ch_tag} {title}: {len(qs)}問（画像{ni}問）')
