# -*- coding: utf-8 -*-
"""
整形外科の章別HTML（整形外科/chNN_*.html）から questions_ortho.json を生成する。
build_uro_json.py と同構造:
  - uid = ortho_ch{nn}_{card_id}（例 ortho_ch01_q1）
  - rate は cr スパンの "45%" から解析
  - 各ファイル=1章、章名は ortho_questions.json から取得
  - 画像 src は "images/xxx" → "整形外科/images/xxx"（study.htmlはルート基準）
"""
...
with open(ORTHO_DIR / 'ortho_questions.json', encoding='utf-8') as f:
    ortho_meta = json.load(f)
chap_names = {c['chapter']['num']: c['chapter']['name'] for c in ortho_meta}

chapters = []
total = n_img = 0
for hf in sorted(glob.glob(str(ORTHO_DIR / 'ch*.html'))):
