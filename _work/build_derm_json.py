# -*- coding: utf-8 -*-
"""
皮膚科の章別HTML（皮膚科/chNN_*.html）から questions_derm.json を生成する。
build_obg_json.py と同構造:
  - uid = derm_ch{nn}_{card_id}（例 derm_ch01_q1）
  - rate は cr スパンの "58%" から解析
  - 各ファイル=1章、章名は derm_questions.json から取得
  - 画像 src は "images/xxx" → "皮膚科/images/xxx"（study.htmlはルート基準）
"""
...
with open(DERM_DIR / 'derm_questions.json', encoding='utf-8') as f:
    derm_meta = json.load(f)
chap_names = {c['chapter']['num']: c['chapter']['name'] for c in derm_meta}

chapters = []
total = n_img = 0
for hf in sorted(glob.glob(str(DERM_DIR / 'ch*.html'))):
