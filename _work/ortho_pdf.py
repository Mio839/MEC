# -*- coding: utf-8 -*-
"""
整形外科PDF（MECマイナー講座・整形外科）から章を作るための抽出ツール。
`_work/uro_pdf.py` の座標系をそのまま流用
（解答一覧表の列xは精神科・皮膚科・眼科・耳鼻咽喉科・泌尿器科と同一）。

  python _work/ortho_pdf.py anstable          巻末解答一覧表(p109-112)を全問ぶん抽出
  python _work/ortho_pdf.py spec   --ch 1     指定章の NO/国試番号/正解/バッジ/正答率 一覧
  python _work/ortho_pdf.py text   --ch 1     指定章の問題ページ本文を dump（Readで読む用）
  python _work/ortho_pdf.py images --ch 1     画像の矩形と設問への帰属候補＋ページ画像（目視確認用）
  python _work/ortho_pdf.py save   --ch 1     images が出した map ファイルに従って画像を保存

出力先は既定で _work/_ortho_tmp/。Bashの標準出力はcp932で潰れるので、
生成された .txt を Read ツールで読むこと。

⚠️ PDF p.4 は「MECマイナー講座の特徴」の見本ページで、載っている問題は精神科のもの。
   整形外科の問題として拾わないこと（本ツールは章ページ範囲で限定しているので混入しない）。
⚠️ 本PDFは各ページの左右端に章名の縦書き（「神」「経」「の」「障」「害」…）が1文字ずつ入る。
...
PDF = os.path.join(BASE, 'MEC問題文pdf', 'MECマイナー講座・整形外科_問題（表紙2026）.pdf')
OUT = os.path.join(BASE, '_work', '_ortho_tmp')
DEST_IMG = os.path.join(BASE, '整形外科', 'images')

TABLE_PAGES = range(109, 113)          # 巻末「解 答」一覧表（印刷p.106-109）
INDEX_PAGES = range(113, 115)          # 巻末「国試番号索引」（印刷p.110-111）
...
CH = {
    ...
    3: (74, 106, 41, 60, '関節疾患'),
    4: (107, 139, 61, 84, '骨　折'),
    5: (140, 166, 85, 102, 'その他の疾患'),
    6: (167, 174, 103, 108, 'ロコモティブシンドロームとサルコペニア・フレイル'),
}

# 解答一覧表の列の左端x（精神科・皮膚科・眼科・耳鼻咽喉科・泌尿器科と同一）
...
def cmd_anstable(_args):
    ...
    ⚠️ 章区切り行（「　2　〔 問題 〕神経の障害」）が NO列・解答列に食い込むため、
...
def cmd_text(args):
    """章の問題ページ本文を dump。
    ⚠️ 左右端の縦書き章名（1文字ずつ縦に並ぶ）を x で落とす。"""
...
def cmd_images(args):
    ...
    ⚠️ 見出し「A 問題（★問題）」「B 問題」の A/B が拾われる。
...
def cmd_save(args):
    """ch{NN}_map.txt に従って 整形外科/images/{国試番号}_{n}.jpeg を保存。
