# -*- coding: utf-8 -*-
"""
公衆衛生の章別HTML（公衆衛生/chNN_*.html）から questions_ph.json を生成する。
build_oph_json.py と同構造:
  - uid = ph_ch{nn}_{card_id}（例 ph_ch01_q1）
  - rate は cr スパンの "45%" から解析
  - 各ファイル=1章、章名は ph_questions.json から取得
  - 画像 src は "images/xxx" → "公衆衛生/images/xxx"（study.htmlはルート基準）
"""
...
with open(PH_DIR / 'ph_questions.json', encoding='utf-8') as f:
    ph_meta = json.load(f)
chap_names = {c['chapter']['num']: c['chapter']['name'] for c in ph_meta}

chapters = []
total = n_img = 0
for hf in sorted(glob.glob(str(PH_DIR / 'ch*.html'))):
