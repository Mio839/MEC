# -*- coding: utf-8 -*-
"""
放射線科PDF（MECマイナー講座・放射線科）から章を作るための抽出ツール。
`_work/anes_pdf.py` の座標系をそのまま流用
（解答一覧表の列xは精神科・皮膚科・眼科・耳鼻咽喉科・泌尿器科・整形外科・麻酔科と同一）。

  python _work/rad_pdf.py anstable          巻末解答一覧表(p39-40)を全問ぶん抽出
  python _work/rad_pdf.py spec   --ch 1     指定章の NO/国試番号/正解/バッジ/正答率 一覧
  python _work/rad_pdf.py text   --ch 1     指定章の問題ページ本文を dump（Readで読む用）
  python _work/rad_pdf.py images --ch 1     画像の矩形と設問への帰属候補＋ページ画像（目視確認用）
  python _work/rad_pdf.py save   --ch 1     images が出した map ファイルに従って画像を保存

出力先は既定で _work/_rad_tmp/。Bashの標準出力はcp932で潰れるので、
生成された .txt を Read ツールで読むこと。

⚠️ PDF p.4 は「MECマイナー講座 放射線科の特徴」の見本ページで、
   放射線診断学の NO.2-6 がレイアウト見本として再掲されている。問題ページ範囲(p5-)で限定して避ける。
⚠️ 本PDFは各ページの左右端に章名の縦書き（「放」「射」「線」「診」「断」「学」…）が1文字ずつ入る。
   本文テキストへ混じるので、cmd_text は x<40 / x>540 の単語を落として dump する。
⚠️ 本PDFは1章が「★問題」→「無印問題」の2ブロックに分かれるが、章としては割らない
   （NO. は章内で連続しているのでそのまま1章として扱う）。
⚠️ 本科目は「画像から診断する」問題が主体で、画像の枚数が他科目より多い。
"""
...
PDF = os.path.join(BASE, 'MEC問題文pdf', 'MECマイナー講座・放射線科_問題（表紙2026）.pdf')
OUT = os.path.join(BASE, '_work', '_rad_tmp')
DEST_IMG = os.path.join(BASE, '放射線科', 'images')

TABLE_PAGES = range(39, 41)            # 巻末「解 答」一覧表（印刷 放Q-36, 放Q-37）
INDEX_PAGES = range(41, 42)            # 巻末「国試番号索引」（印刷 放Q-38）

# 章 -> (NO.開始, NO.終了, 問題ページ開始, 問題ページ終了, 章名)
CH = {
    1: (1, 1, 5, 6, '序　章'),
    2: (2, 34, 7, 28, '放射線診断学'),
    3: (35, 44, 29, 32, '放射線治療学'),
    4: (45, 60, 33, 38, '医療安全・放射線防護'),
}

# 解答一覧表の列の左端x（精神科・皮膚科・眼科・耳鼻咽喉科・泌尿器科・整形外科・麻酔科と同一）
...
def cmd_anstable(_args):
    ...
    ⚠️ 章区切り行（「　2　〔 問題 〕放射線診断学」）が NO列・解答列に食い込むため、
    ...
    txt.append('ANS BROKEN: %s' % [r['no'] for r in rows
                                   if not re.match(r'^[a-i](,[a-i])*$', r['ans'].strip())])
    ...
    txt.append('--- 全行 ---')
    for r in rows:
        txt.append('NO.%-3s ch%s ans=%-7s kid=%-10s type=%-3s cbt=%-2s irt=%-3s '
                   'hisshu=%-2s ippan=%-2s rinsho=%-2s rate=%-5s theme=%s / %s'
                   % (r['no'], r['ch'], r['ans'], r['kid'], r['type'], r['cbt'],
                      r['irt'], r['hisshu'], r['ippan'], r['rinsho'],
...
def cmd_spec(args):
    ...
    for r in _rows(args.ch):
        ...
        if '○' in r['hisshu']:
            b.append('bh')
        lines.append('NO.%-3d %-10s ans=%-7s badges=%-12s rate=%-5s theme=%s / %s'
...
def cmd_images(args):
    ...
    ⚠️ 講義用の「参考画像」が問題の傍らに置かれることがある。
       これは設問の図ではないので map から手で外すこと。
    """
...
def cmd_save(args):
    """ch{NN}_map.txt に従って 放射線科/images/{国試番号}_{n}.jpeg を保存。
