...
//    （2026-09-08に是正して 131種 → 138種）。**種類の数は減らないはず**なので、
//    ここが減ったら枝番かステムのどちらかを取り落とした合図。
test('別冊の図の索引が壊れていない（122問・138種）', () => {
  const withFig = D.questions.filter(q => q.fig);
  assert.strictEqual(withFig.length, 122, 'fig を持つ設問数');
  const pairs = new Set();
  withFig.forEach(q => {
  ...
  assert.strictEqual(pairs.size, 138, '(ブロック, 別冊No) の種類');
});
