# -*- coding: utf-8 -*-
"""
questions_tox.json を新科目HTML生成ガイド §2（品質基準）・§4（採点/画像の不変条件）で検査する。
`_work/verify_rad_ch.py` の中毒・職業病版。

  python _work/verify_tox_ch.py            全章
  python _work/verify_tox_ch.py --ch 1     指定章のみ

さらに巻末解答一覧表（_work/_tox_tmp/anstable.json）と
 正解ラベル・正答率・バッジ・問題数 を突合する（PDFが正本）。
"""
import argparse, io, json, os, re, sys

BASE = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
JSON = os.path.join(BASE, 'questions_tox.json')
ANS = os.path.join(BASE, '_work', '_tox_tmp', 'anstable.json')
FW2A = {'ａ': 'a', 'ｂ': 'b', 'ｃ': 'c', 'ｄ': 'd', 'ｅ': 'e', 'ｆ': 'f', 'ｇ': 'g', 'ｈ': 'h', 'ｉ': 'i'}
...
def main():
    ...
    if not os.path.exists(ANS):
        # _work/_rad_tmp/ は .gitignore 済み＝別マシンでは存在しない。作り直す。
        print('%s が無い。先に `python _work/tox_pdf.py anstable` を実行すること。' % ANS)
        return 1
    ...
    for ci, ch in enumerate(d['chapters'], 1):
        ...
        for q in ch['qs']:
            ...
            if r is None:
                ...
                # （`※採点除外` のほか、ortho NO.124 は `※不正解者のみ採点除外`／anes NO.45 は `※採点除外`）。
                ...
                # ⚠️ 本PDFの解答一覧表には★（種別）列・CBT列が無いので、
                #    突合できるバッジは bh（必修）だけ。bs/bc が付いていたらそれ自体が誤り。
                want_b = '○' in r['hisshu']
                if want_b != ('bh' in badge_cls):
                    errs.append('%s: バッジ bh の不一致 HTML=%s PDF=%s'
                                % (uid, 'bh' in badge_cls, want_b))
                for cls in ('bs', 'bc'):
                    if cls in badge_cls:
                        errs.append('%s: 本PDFに無いバッジ %s が付いている' % (uid, cls))
