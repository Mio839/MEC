# -*- coding: utf-8 -*-
"""
泌尿器科の章別HTML（泌尿器科/chNN_*.html）から questions_uro.json を生成する。
build_ent_json.py と同構造:
  - uid = uro_ch{nn}_{card_id}（例 uro_ch01_q1）
  - rate は cr スパンの "45%" から解析
  - 各ファイル=1章、章名は uro_questions.json から取得
  - 画像 src は "images/xxx" → "泌尿器科/images/xxx"（study.htmlはルート基準）
"""
...
with open(URO_DIR / 'uro_questions.json', encoding='utf-8') as f:
    uro_meta = json.load(f)
chap_names = {c['chapter']['num']: c['chapter']['name'] for c in uro_meta}

chapters = []
total = n_img = 0
for hf in sorted(glob.glob(str(URO_DIR / 'ch*.html'))):
