# -*- coding: utf-8 -*-
"""
精神科の章別HTML（精神科/chNN_*.html）から questions_psy.json を生成する。
build_obg_json.py と同構造:
  - uid = psy_ch{nn}_{card_id}（例 psy_ch01_q1）
  - rate は cr スパンの "58%" から解析
  - 各ファイル=1章、章名は psy_questions.json から取得
  - 画像 src は "images/xxx" → "精神科/images/xxx"（study.htmlはルート基準）
"""
...
with open(PSY_DIR / 'psy_questions.json', encoding='utf-8') as f:
    psy_meta = json.load(f)
chap_names = {c['chapter']['num']: c['chapter']['name'] for c in psy_meta}

chapters = []
total = n_img = 0
for hf in sorted(glob.glob(str(PSY_DIR / 'ch*.html'))):
