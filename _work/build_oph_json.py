# -*- coding: utf-8 -*-
"""
眼科の章別HTML（眼科/chNN_*.html）から questions_oph.json を生成する。
build_derm_json.py と同構造:
  - uid = oph_ch{nn}_{card_id}（例 oph_ch01_q1）
  - rate は cr スパンの "45%" から解析
  - 各ファイル=1章、章名は oph_questions.json から取得
  - 画像 src は "images/xxx" → "眼科/images/xxx"（study.htmlはルート基準）
"""
...
with open(OPH_DIR / 'oph_questions.json', encoding='utf-8') as f:
    oph_meta = json.load(f)
chap_names = {c['chapter']['num']: c['chapter']['name'] for c in oph_meta}

chapters = []
total = n_img = 0
for hf in sorted(glob.glob(str(OPH_DIR / 'ch*.html'))):
