# -*- coding: utf-8 -*-
"""
麻酔科の章別HTML（麻酔科/chNN_*.html）から questions_anes.json を生成する。
build_ortho_json.py と同構造:
  - uid = anes_ch{nn}_{card_id}（例 anes_ch01_q1）
  - rate は cr スパンの "45%" から解析
  - 各ファイル=1章、章名は anes_questions.json から取得
  - 画像 src は "images/xxx" → "麻酔科/images/xxx"（study.htmlはルート基準）
"""
...
with open(ANES_DIR / 'anes_questions.json', encoding='utf-8') as f:
    anes_meta = json.load(f)
chap_names = {c['chapter']['num']: c['chapter']['name'] for c in anes_meta}

chapters = []
total = n_img = 0
for hf in sorted(glob.glob(str(ANES_DIR / 'ch*.html'))):
