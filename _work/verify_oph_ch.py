# -*- coding: utf-8 -*-
"""
questions_oph.json を新科目HTML生成ガイド §2（品質基準）・§4（採点/画像の不変条件）で検査する。

  python _work/verify_oph_ch.py            全章
  python _work/verify_oph_ch.py --ch 5     指定章のみ

さらに巻末解答一覧表（_work/_oph_tmp/anstable.json）と
 正解ラベル・正答率・バッジ・国試番号 を突合する（PDFが正本）。

⚠️ このファイルは 2026-08-26 に書き直した。
   `verify_ent_ch.py` の docstring が「`_work/verify_oph_ch.py` の耳鼻咽喉科版」と
   名指ししているとおり本来は眼科版が先にあったはずだが、実体は **0 バイト**で、
   眼科だけ照合スクリプトが存在しない状態が続いていた（CLAUDE.md は「抽出・照合の道具」
   として名前を挙げていたので、無いことに気づけなかった）。ent 版を土台に復元してある。

⚠️ 眼科の解答一覧表には他科目に無い **「疾患名」列（`dis`）** がある。これは眼底写真の読みを
   裏取りする正本で（CLAUDE.md 眼科の行を参照）、JSON 側に対応する項目が無いので
   突合はできない。`--dis` で一覧を出せるようにしてあるので、章を書くときに目視で使う。
"""
import argparse, io, json, os, re, sys

BASE = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
JSON = os.path.join(BASE, 'questions_oph.json')
ANS = os.path.join(BASE, '_work', '_oph_tmp', 'anstable.json')
# ⚠️ 5択決め打ちにしないこと。この教材には 6択・7択・9択が実在する。
FW2A = {'ａ': 'a', 'ｂ': 'b', 'ｃ': 'c', 'ｄ': 'd', 'ｅ': 'e',
        'ｆ': 'f', 'ｇ': 'g', 'ｈ': 'h', 'ｉ': 'i'}
...
def main():
    ...
    ap.add_argument('--dis', action='store_true',
                    help='解答一覧表の「疾患名」列を NO. 順に出す（眼底写真の読みの裏取り用）')
    a = ap.parse_args()

    d = json.load(io.open(JSON, encoding='utf-8'))
    if not os.path.exists(ANS):
        # _work/_oph_tmp/ は .gitignore 済み＝別マシンでは存在しない。作り直す。
        print('%s が無い。先に `python _work/oph_pdf.py anstable` を実行すること。' % ANS)
        return 1
    ...
    if a.dis:
        for no in sorted(rows):
            r = rows[no]
            print('NO.%-4d %-12s %s' % (no, r['kid'], r.get('dis', '')))
        return 0
    ...
    for ci, ch in enumerate(d['chapters'], 1):
        ...
        for q in ch['qs']:
            ...
            # 計算問題（桁入力）は選択肢を持たない＝ok 0個が正しい
            is_calc = not q['choices'] and '計算答：' in (q.get('ans_label') or '')
            if n_ok == 0 and not excluded and not is_calc:
            ...
            if not excluded and not is_calc and n_ok != want:
            ...
            if r is None:
                ...
                # 「※不正解者のみ採点除外」等の注記は落として比べる
                kid = re.sub(r'※.*$', '', r['kid']).strip()
                if kid != q['episode'].strip('()'):
                ...
                if not is_calc and pdf_ans != got:
