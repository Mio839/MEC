...
RE_BETSU = re.compile(r'別\s*冊\s*No\s*\.\s*(\d+)\s*((?:[A-Z]\s*(?:、\s*)?)*)')
# ⚠️⚠️ **「別」と「冊」の間で行が折れる紙面がある**（実測6か所）。`別冊` を隣接で書くと
#    その問の fig が丸ごと落ちる——2026-09-09 に A42・C14・C57・D66・E49・E50 の6問が
#    図無しになっていた（エラー報告 m121s_ch03_q139）。E37 だけは同じ別冊No. が
#    後ろのページにも出ていたので偶然助かっていた。
