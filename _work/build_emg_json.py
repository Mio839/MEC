# -*- coding: utf-8 -*-
"""
救急の章別HTML（救急/chNN_*.html）から questions_emg.json を生成する。
build_rad_json.py と同構造:
  - uid = emg_ch{nn}_{card_id}（例 emg_ch01_q1）
  - rate は cr スパンの "45%" から解析
  - 各ファイル=1章、章名は emg_questions.json から取得
  - 画像 src は "images/xxx" → "救急/images/xxx"（study.htmlはルート基準）
  - 設問図は19枚18問。連問の共通ステムの図は「そのステムを表示する兄弟全員」に付く
"""
...
with open(EMG_DIR / 'emg_questions.json', encoding='utf-8') as f:
    emg_meta = json.load(f)
chap_names = {c['chapter']['num']: c['chapter']['name'] for c in emg_meta}

chapters = []
total = n_img = 0
for hf in sorted(glob.glob(str(EMG_DIR / 'ch*.html'))):
