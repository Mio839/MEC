# -*- coding: utf-8 -*-
"""
内分泌PDF（MEC臓器別講座・内分泌代謝・281ページ）から設問の原文・画像を取り出し、
questions_endo.json と突き合わせる。circ_pdf.py の同型（版面が同じ臓器別講座なので
解答一覧表の列座標まで一致する: 出題テーマ@263.7 / 疾患名@405.4 / 必修○@477 / 一般○@494 / 臨床○@511）。

    python _work/endo_pdf.py pages                  各章がPDFの何ページかを推定
    python _work/endo_pdf.py text   --ch 1          その章の設問原文をdump（Readで読む用）
    python _work/endo_pdf.py qtdiff --ch 1          JSONのqtとPDF原文の乖離を検出
    python _work/endo_pdf.py images --ch 1          画像矩形＋帰属候補＋ページ描画を出す
    python _work/endo_pdf.py save   --ch 1          ch01_map.txt を検収してから保存

⚠️ 帰属は機械では決めきれない＝**整形外科式**（機械が候補を出し、ページ描画を見て
   ch{NN}_map.txt を1行ずつ検収してから save）。産婦人科式の全自動は使えない
   （has_image に相当する独立した権威が呼吸器には無い）。

⚠️ qtdiff の「要約の疑い」は当てにならない（循環器で全10章 0件だったのに ch06 は
   実際には22件の食い違いを抱えていた）。連問ステムの欠落は字数比に出ないため。
...
PDF = os.path.join(BASE, 'MEC問題文pdf', 'MEC臓器別講座・内分泌代謝_問題（表紙2026）(1).pdf')
JSON_PATH = os.path.join(BASE, 'questions_endo.json')
OUT = os.path.join(BASE, '_work', '_endo_tmp')
DEST_IMG = os.path.join(BASE, '内分泌', 'images')
...
def cmd_save(a):
    """ch{NN}_map.txt に従って 内分泌/images/{国試番号}_{n}.jpeg を保存。
