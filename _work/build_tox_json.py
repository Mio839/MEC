# -*- coding: utf-8 -*-
"""
中毒・職業病の章別HTML（中毒・職業病/chNN_*.html）から questions_tox.json を生成する。
build_rad_json.py と同構造:
  - uid = tox_ch{nn}_{card_id}（例 tox_ch01_q1）
  - rate は cr スパンの "45%" から解析
  - 各ファイル=1章、章名は tox_questions.json から取得
  - 画像 src は "images/xxx" → "中毒・職業病/images/xxx"（study.htmlはルート基準）
  - 本科目に画像は0枚（PDF唯一の埋め込み図はレジュメの解説図なので設問に帰属させていない）
"""
...
with open(TOX_DIR / 'tox_questions.json', encoding='utf-8') as f:
    tox_meta = json.load(f)
chap_names = {c['chapter']['num']: c['chapter']['name'] for c in tox_meta}

chapters = []
total = n_img = 0
for hf in sorted(glob.glob(str(TOX_DIR / 'ch*.html'))):
