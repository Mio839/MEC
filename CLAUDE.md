...
従来の演出と実機で比べるための**切り替えスイッチ**。既定は従来のまま。切り替えは統合学習ツールの
🔊効果音 →「演出スタイル」（UIローカル・同期しない・**試験中は切り替え不可**）。
正本は `study_exam.js` 末尾の「新しい演出（試作）」節（`_fxRefined()` / `_rf*`）＋ `study.css` 末尾。
**従来の経路は1行も消していない**——3つの採点経路の正解演出は `if (_fxRefined()) {…} else` で分岐しているだけ。

- **正解**：同じ量の演出を「正解の肢から、順番に」出す（0ms 肢の縁を光が1周 → 80ms 光が数字へ走る →
  200ms UIテーマ固有演出を**肢の位置で** → 240ms 連続数 `#examRfStreak`）。全画面フラッシュ・画面の揺れ・
  浮遊コンボ・外周パルス・散らばった位置の粒子は出さない。暗転は**段が上がった瞬間だけ**。
  ⚠️ 連続数は `#examStreakToast` を使わず専用要素（UIテーマ側の上書きが多いため）。書体・枠はUIテーマ8種ぶん。
- **誤答＝答えを見せずに選び直させる**（ユーザー判断）。1回目の誤答で採点・記録を済ませ（不正解）、
  ...
  ⚠️ 数字キーは refined では `.click()` に流して同じ経路を通す（キーボード側に分岐を複製しない）。
...
- 採用・不採用が決まったら、不採用側の経路とこの節を消すこと（2本立てを恒久化しない）。
