...
- **選択肢データの欠落14問（科目2・過去問12）** — 図のa〜eや組合せの選択肢がデータに無い。
  計算問題ではないので入力型では解決しない。試験モードでは中立で開封して先へ通す
  （`_isExamUngraded`）、過去問側は出題から外して理由を表示する（`ceIsAnswerable`）。
  当初26問あり、うち12問はPDFから復元済み（下記）。残り14問は選択肢が**表または図**で、
  テキストとして取り出すと列の対応を誤るため手を付けていない。スクリーンショットが必要。
