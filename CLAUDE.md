...
| `耳鼻咽喉科/` | マイナー講座・耳鼻咽喉科（prefix `ent`・👂・#0F766E）。章別HTML(`ch01_mimi_kihon.html`〜)＋`images/`＋`ent_questions.json`（全8章の章名メタ）。HTMLが`questions_ent.json`のソース＝章ごとの生成器`_work/build_ent_ch{NN}.py`→`_work/build_ent_json.py`で再生成。眼科と同方式。**全8章214問・章頭NO.は 1／21／51／99／102／138／152／172**（章名はPDF表記のまま「耳①：耳の基本」等）。**2026-08-06に第1章「耳①：耳の基本」NO.1-20（画像8問14枚）を追加**。
⚠️ **NO.19(111I-40) は a〜g の7択**（純音聴力検査の結果①〜⑦から選ぶ）。5択決め打ちのコードを書かないこと——生成器の `FW` は f/g まで持つ。
⚠️ **図中の①〜⑤／①〜⑦が選択肢そのものになる問題がある**（ch01は NO.10・15・19）。PDFを300〜600dpiで拡大して目視で同定すること。
...
| 眼科（マイナー講座） | oph | 8 | 213 |
| 耳鼻咽喉科（マイナー講座） | ent | 1／8 | 20／214（進行中） |
| 実力試験Ⅰ | jitsu1 | 2 | 160 |
...
※ 耳鼻咽喉科は2026-08-06に第1章20問を追加して着手。**全8章214問の予定**（耳20/30/48・鼻口唾3/36/14・咽喉頭20/43）。`study.html` の科目ヘッダは実数（現在20問）を出す。
