...
| 必修講座 | `hisshu`（解説は既存科目からの借用＋手書きの上書きだけ・Part2 は同じ科目で章を続ける） |
...
- 科目prefix（全28）: `endo` / `resp` / `circ` / `dige` / `neur` / `hbp` / `jinzo_d` / `hema` / `imma` / `kansen` / `peds` / `obg` / `psy` / `derm` / `oph` / `ent` / `uro` / `ortho` / `anes` / `rad` / `tox` / `emg` / `ph` / `hisshu` / `m121s` / `jitsu1` / `custom` / `memo`
...
- ⚠️ **採点除外は `mecOnLapSRS` でも弾くこと**。試験モードの3つの採点経路はすべて
...
`_updateSRS` は**経過日数を一度も見ていなかった**。間隔は無条件に `前回interval × ef` で
伸びるので、**同じ日に同じ問題を6回正解しただけで間隔が90日＝上限に達し、復習キューから
事実上消えた**（実ソースで実測）。科目を絞って試験モードを繰り返すのが通常の使い方なので
日常的に起きていた＝間隔反復の「間隔」が一度も測られていなかった。

`ratio = 経過日数 / 予定間隔`（0〜1で頭打ち）で**伸びしろだけ**を按分する:
`interval = prev + (計算値 - prev) × ratio`、`ef` の +0.1 も同じ ratio を掛ける。

- `ratio=1`（予定どおり以降に解いた）→ **従来と1日も違わない値**になる。
- `ratio=0`（同じ日の解き直し）→ 間隔も ef も据え置き。
- ⚠️ **`ng` は割り引かない**（経過に関わらず翌日へ戻す）。早すぎる失敗も失敗である。
- ⚠️ **`lastSeen` を持たない旧データは `ratio=1`（満額）とみなす**。既存の進捗を巻き戻さないため。

### ⚠️ 間隔のゆらぎ（`_srsFuzz`・2026-09-06〜）

予定日が完全に決定論だったため、ある日100問を初回正解すると**翌日に100問、その6日後にも
同じ100問が塊のまま戻っていた**（実測: 45日先の due は平均37問/日に対し標準偏差17.9・最大91）。
`SRS_FUZZ_PCT = 0.05` で ±5%（最低±1日）ずらす。

- ⚠️⚠️ **乱数を使わないこと。** `uid` と間隔だけから決まる値にしておけば、どの端末で解いても
  同じ `nextReview` になる＝Gist同期でどちらのエントリが勝っても予定日が動かない。
  `Math.random()` にすると同期のたびに復習予定日が揺れて stats.html の「見通し」が信用できなくなる。
- ⚠️ **ゆらぎは「伸びた時だけ」当てる**（`interval > prev` のときだけ）。据え置きの回にも当てると、
  同じ間隔には必ず同じ方向の値を返すので、同日に解き直すたび一方向へ乱歩する。
- 1〜2日はずらさない（0日や負にできない）。

### ⚠️ 間隔は試験日を越えない（`_srsExamCap`・2026-09-07〜）

`_updateSRS` は**試験日を一度も見ていなかった**。SM-2 も Anki も「無期限の保持」を前提にした
式なので試験日という概念を持たないが、本ツールは締切が固定されている。そのため
**残り日数より長い間隔が付いた瞬間、その問題は試験まで二度と出題されなくなっていた**。

実測（実ソースを仮想時計で回した）: 2026-09-07 起点・全問○を予定どおりで `1 → 5 → 12 → 31 → 74 → 90` 日。
成熟札（interval 90）を **2026-11-21** に復習すると次回は `2027-02-19` ＝ 試験（2027-02-06）の13日後。
**11月上旬を境に、仕上がっている問題から順に復習キューから抜け落ちる**——直前期にいちばん
見直したいものが消えるという向きだった。

`SRS_EXAM_FRACTION = 0.5` で `interval ≤ floor(残り日数 × 0.5)` に頭打ちにする。

- ⚠️⚠️ **「残り日数−1」のような目一杯の上限にしないこと。** 成熟札が軒並み同じ日
  （試験の直前）へ寄って、最終日に数千問の山ができる。半分なら1枚の軌跡が
  `1 → 5 → 12 → 31 → 51 → 25 → 12 → 7 → 4 → 2 → 1`（2026-09-07 起点・全問○）と
  **伸びてから幾何級数的に詰まり、直前期ほど自然に回転が上がる**（試験までの出題回数は
  1問あたり 6回 → 12回）。⚠️ その裏返しで**復習の在庫は増える**。式ではなく `SRS_EXAM_FRACTION` 1つで
  加減できるようにしてあるので、重ければここを下げる。
- ⚠️ **ゲートは `_srsFuzz` より前に掛けること。** 後に掛けると上限で切られた札が全部きっかり
  同じ値になり、ゆらぎで散らした効果が消える（塊で戻ってくる＝`_srsFuzz` を入れた理由が再発）。
- ⚠️ **ゆらぎは「伸びた回」だけでなく「上限で切られた回」にも当てる**（`interval > prev || _capped`）。
  直前期は成熟札が軒並み上限へ落ちるので、ここを外すとその日に解いた札が全部同じ予定日に着地する
  （実測: 50枚が1日に集中 → 3日へ分散）。切られた回は次も必ず上限で切り直されるので、
  据え置き時のような一方向の乱歩にはならない。
- ⚠️ **試験日を過ぎている（または過去日が設定されている）ときはゲートを掛けない。**
  残り0日で全問が interval 1 に潰れるのを避けるため、その場合は従来どおり 90 日上限のみ。
- ⚠️ **試験日の正本は `progress.js` の `MECSync.examDate()`**（`mec_exam_date_v1`・既定 `2027-02-06`）。
  stats.html の国試カウントダウンと同じ値を読む＝**ページ側に既定値を持たせないこと**
  （片方だけ動かすと「カウントダウンが指す日」と「復習予定を切り詰める日」が食い違う）。
  ⚠️ このキーは**同期対象ではない**（`_mergeRemote` は文字列スカラの解決戦略を持たない）。
  既定のままなら全端末で一致するが、片方で変更すると端末間で `nextReview` がずれる
  ＝`_srsFuzz` が守っている「どの端末でも同じ予定日」がそこだけ崩れる。同期させるなら
  更新時刻を添えた last-writer-wins を作る必要がある。

### ⚠️ 復習キューの並びは「待たされ具合」の降順（2026-09-06〜）

`startSRSReview` は due を `SRS_SESSION_LIMIT`(=50) で切るので**どの50問かが学習効果を決める**。
2026-09-06 まで `nextReview` の昇順（＝古い順）で、遅れが溜まると純粋なFIFOになり、
**昨日おぼえたばかりの間隔1日の問題が1か月前の期限切れ数千問の後ろに並んだ**
＝いちばん忘れかけている問題が構造的に後回しになっていた。

`_srsUrgency(e, today) = (遅れ日数 + 1) / 予定間隔`。間隔1日が1日遅れる（2.0）方が、
間隔90日が5日遅れる（1.07）より切迫している。同着は `ef` 昇順（従来どおり）。

- 実ソースを仮想時計で180日回した結果: 出題8950問のうち「間隔≤2日」のもろい問題が
  **63.8% → 85.4%**、見送ったもろい問題が延べ **322,206 → 138,476件**。
- ⚠️ **並び替えではバックログ自体は減らない**。1日100問の目標が SRS エントリを1日約100件生み、
  消化枠は50件なので、遅れは構造的に増える。上限を上げるか1日2セッション目を許すかは別の判断。

### ⚠️ 出題順は科目が交ざるように並べ替える（`_srsInterleave`・2026-09-06〜）

`_renderDueCardsForReview` は科目ごと（`bySid`）にまとめて描き、`_buildExamQueue` は
`.sg` を持たないカードを入力順のまま返すので、**復習セッションは科目ブロック順で出ていた**。
2026-09-06 まで「全dueをシャッフルしたまま出せるので科目ミックスの復習効果も保たれる」と
コメントに書いてあったが、**シャッフルはコード中に1箇所も無かった**。

- **選抜（どの50問か）は上の決定論のまま、出す順だけを uid と日付のハッシュで交ぜる。**
- ⚠️ **ホストのDOM順も一緒に並べ替えること**。uid の並びだけでも出題順は変わるが、
  DOM順が科目ブロックのままだと `_scrollToNextCard` がページを前後に飛び回る。
- ⚠️ 乱数にしないこと。同じ日に開き直したとき「さっき出た問題がまた先頭」に見える。

### ⚠️ 交ぜる単位は「連問の群」（`_seriesInfoOf` / `_seriesGroupsOf`・2026-09-07〜）

`_srsInterleave` は uid を**1問ずつ**ハッシュで交ぜていたので、**連問（1つの症例を複数の設問で
追うもの・2026-09-07 の実測で全科目 288群726問）は復習キューで必ずバラバラの順に出ていた**。「Q.107 → Q.106」と
症例の途中から始まるうえ、**後の設問が前の設問の答え（診断名・実施した処置）を先に見せる**。
産婦人科は2問目以降が「（上記のFGR症例。）」と前を参照する書き方なので、逆順だと読めない。

- **群の中は常に章 → 番号の昇順に固定し、交ぜるのは群と群だけ。** 群の代表は並べ替え後の1問目
  ＝連問を含まないセッションでは**従来と1問も違わない並び**になる（テストが突き合わせる）。
- ⚠️⚠️ **連問であることを示す印はデータにも DOM にも無い。** `_buildExamQueue` が見る `.sg`
  （Series group）は `card_renderer.js` が一度も出力しない＝**JSON描画では実質デッドコード**で、
  連問がまとまるのは通常の試験モードが DOM順（＝uid順）で出しているからにすぎない。
  したがって判定材料は **qt の書式だけ**で、書式は3つある:

  | | 書式 | 群の識別子 |
  |---|---|---|
  | ① | 宣言文「次の文を読み、40 と41 の問いに答えよ。」（作り直した科目） | 番号（科目内で一意） |
  | ② | `<span class="qt-context">` に共通ステム（旧コア12科目） | 本文そのもの |
  | ③ | 兄弟が要約だけを持つ（産婦人科）「（19歳の女性。…）」 | ①を持つ1問目から番号で引き寄せる |

- ⚠️ **①の番号を uid の番号として信用してよいのは「自分の番号が範囲に入っているとき」だけ。**
  `imma` は章ごとに番号が振り直されていて、宣言文は PDF の NO.203-204 なのに uid は `_q6`/`_q7`。
- ⚠️ **番号による同定は必ず同じ章の中だけで行う**（`imma` と `memo` は同じ番号が複数の章にある）。
  実データでは**章をまたぐ群は1つも無い**ので、章で名前空間を切って安全側に倒せる。
- ⚠️ ②のステムは**「連問 n/m」ラベルと、科目によって本文に混ざる宣言文の両方を落としてから**
  比べること（落とさないと1問目だけ別の群になる）。**空白は全部落とす**——空白の有無だけが違う
  兄弟が実在する（kansen NO.189/190）。
- **上限50問の境目で切れた兄弟は、今日 due なものだけ拾い直す**（`_srsSeriesPullIns`）。
  ⚠️ **due でない兄弟まで出さないこと**＝SRSの予定を先取りすることになる。判定はカードの qt を
  読むので**描画の後でしかできない**（`_renderDueCardsForReview` は「足す」関数なので追加で呼ぶ）。
  拾ったぶんセッションは50問を超える（`SRS_SERIES_PULL_MAX = 10` ＝最大60問。0 で無効）。
  兄弟は同じセッションで解かれる＝間隔も ef も揃うので、実際に起きるのは境目に跨ったときだけ。
- ⚠️ **SRSのエントリは uid 単位のまま**（連問をまとめて1つにはしない）。揃って due になるとは
  限らないので、**同じ日に兄弟が来ない日はある**——これは仕様。
- テストは実データ全数（`questions_*.json` 25科目）を流して3書式から群を復元できることを見る。
  ⚠️ **群の数（288）や問題数（726）を書き写さないこと**——章を足せば変わる。テストが見るのは
  「1問だけの群が無い・番号が連番・章をまたがない」という性質と下限だけ。
...
### 復習キューの意匠（`.hero-say.srs-card`・2026-09-08〜）

ハブの復習キューは全8UIテーマぶんの意匠を持つ（Aurora=プリズム結晶／Brass=ギヨシェ彫りの真鍮銘板＋ビス留め／
Cyber=角を落とした戦術HUD＋走査線／Liquid=ラバランプのガラスチャンバー＋気泡／Kintsugi=漆黒陶器の金継ぎ／
Celestial=星図と金の菱星／Abyss=耐圧殻＋ソナー／Frost=六花の氷結板）。全部 `index.html` のインライン CSS。

- ⚠️⚠️ **stat 行（`.srs-card-stat`）の1行表示は死守すること**（2026-09-07r で2行落ちを根絶した）。
  だから意匠は **`position:absolute` の疑似要素だけ**で置き、**横方向の寸法を1pxも増やさない**。
  stat 行の中に飾りの要素を足さないこと。
- ⚠️ **使える層は5枚だけで分担が固定**（同じ層に2つ置くと後勝ちで片方が黙って死ぬ）:
  `.hero-say::before`＝地の模様(z0)／`.hero-say::after`＝上に載る光沢・走査(z3)／
  `.srs-deco`＝内枠(z1)／`.srs-deco::before`＝左上の飾り／`.srs-deco::after`＝右下の飾り。
- ⚠️ **既定は全部 `opacity:0`**。テーマ側で `opacity:1` にして初めて出る＝テーマを増やしても
  「素の見た目」が勝手に変わらない。
- ⚠️ **右下の飾りとリスク帯（`.srs-risk-bar`）は当たる**。帯の左右マージン18pxがその逃げ＝縮めないこと。
- ⚠️ Cyber だけ `clip-path` で角を落としている。**`clip-path` は `box-shadow` ごと切り落とす**ので、
  外側の光は `filter:drop-shadow` で出している（`box-shadow` に戻すと光が消える）。
- ⚠️ **reduced-motion で `display:none` にしないこと**。止めるのは走査・光沢・明滅・ベルの揺れだけで、
  枠・彫り・四隅の飾り・リスク帯は「動き」ではなく情報と質感なので残す。
- ⚠️ テーマが `.srs-card-ttl` に `background-clip:text` を掛けるので、見出しの絵文字は
  `.srs-ttl-ic` に包んで `-webkit-text-fill-color:initial` を戻す（包み忘れると絵文字が透明になる）。
  ピルの絵文字も同じ理由で `.srs-pi`。
...
## 📋 今日の所見（2026-09-12・旧「臨床スキルプロファイル」の置き換え）

ハブのスパークラインの下。その日のデータから毎回作り直す**3件の観察**を出す。
実装は `index.html` のみ（`_noteFacts` / `_noteChapterFacts` / `_noteSrsFacts` / `_buildHubNotes` /
`_pickHubNotes` / `_renderHubNotes`）。テスト: `node _work/test_hub_notes.js`（29件・実ソースを vm で回す）
＋ `node _work/test_hub_fx.js`（テーマ・reduced-motion・回帰ガード）。

### ⚠️⚠️ 六角レーダー（`.skill-radar-box`）を戻さないこと

旧実装の6軸は **`今日の正答率 × 固定係数 [1, 1.06, .94, 1.08, .92, 1.04]`** で、
**形が原理的に変わらず大きさだけが伸縮する図**だった。「循環器」「呼吸器」…のラベルが
付いていたが**どの軸もその科目のデータを1ビットも読んでおらず**、今日まだ解いていない日は
`0.55` 固定＝毎朝まったく同じ絵が出ていた。「弱点スキャン中」も一度も書き換わらない固定文字列。
文字が切れていたのは viewBox 160 に対し `x=142 anchor=start`（約167px）と
`x=18 anchor=end`（約 −20px）が枠外へ出ていたため。`test_hub_fx.js` が残骸の復活を検出する。

⚠️ **科目×何かのプロファイルを作りたくなったら stats.html の弱点カルテへ足すこと。**
あちらは**同じ uid どうしで全国正答率と比べている**ので、ハブに劣化版を置く意味がない。

### ⚠️ 追加のファイル読み込みを1つも増やさない

材料は localStorage と、ハブが既に読んでいる `chapters_meta.js`（`MEC_CHAPTERS`）/
`mindmap_data/index.js`（`MM_SUBJECTS`）だけ。**`qmeta.json`(564KB)・模試の解答表(107KB) を
持ち込まないこと**——ハブを開くたびにその重さを払うことになる
（🩺タイルの脚に得点を出さない約束と同じ理由）。`test_hub_fx.js` が `<script src>` と
`fetch()` を見張る。
⚠️ `rate_index.js` だけは 2026-09-12 から**実力レーダーのために意図的に読み込む**
（32KB・gzip 12KB ＝ `chapters_meta.js` と同程度。→ 全国正答率の索引の節）。
所見の側はこれを読まない——所見は行動の観察で、全国比は実力レーダーとカルテの担当。

### 所見のルール（16本）

`_buildHubNotes` が候補を作り、`_pickHubNotes` が3件選ぶ。1件 = `{id, w, tone, ic, tx, href, cta}`。

| 材料 | 出る所見 |
|---|---|
| `mec_attempts_v1` の `n`（セッション内順） | 後半の失速／持続（前半・後半とも40問以上のときだけ） |
| 同 `sec` | 5秒未満の誤答＝読み飛ばし／60秒以上考えて落とす科目＝知識の穴 |
| 同 時刻(JST) | 22時以降と日中の差（各30問以上） |
| 同 科目別 直近7日 vs その前7日 | 伸びている科目／落ちている科目（両期間とも10問以上） |
| 同 「前に落とした uid を解き直したか」 | 解き直しの成否（8問以上） |
| 同 日別 | 今日の正答率が直近14日の自己ベスト／通算との差 |
| `done_v2` × `MEC_CHAPTERS` | あと N問で終わる章 |
| `mec_srs_v1` | 期限切れと最も古い予定／明日の件数／間隔30日以上に育った札 |
| `mec_choice_v1` | 同じ誤答肢を2回以上選んだ問題数 |
| 連続日数 | 節目ちょうど／あと2日以内 |

- ⚠️ **どのルールにも最低件数（n）を必ず置くこと。** 3回中0回のような小さい分母から「傾向」を
  名指しすると、偶然を指摘として読ませることになる。
- ⚠️ **「あと N問で終わる章」は済 50% 以上の章だけ**。`left <= 15` だけで拾うと、
  **1問も手を付けていない小さい章（count ≤ 15）が「あと N問で終わる」に化ける**（実際に踏んだ）。
- ⚠️ **ペースバナー（`#heroPaceBanner`）と同じことを言わないこと**。あちらは「直近3日の
  問題数 vs 先週」＝量の話で、所見は正答率・時間・順番の話に寄せてある。
- ⚠️ 正誤が残るのは**試験モード・SRS復習・章別試験だけ**。通常モードの「済」は正誤を持たない
  （`done_v2` は uid→周回数）ので、通常モードだけで解いた日は正答率系の所見が出ない。これは仕様。

### ⚠️ 選抜は決定論。乱数を使わない

`mec_hub_notes_v1`（**UIローカル・非同期**）に `{day, ids, seen:{id→出した日}}` を持つ。

- **同じ日のうちは並びが動かない**（`ids` に載っている候補へ +30）。`renderHero()` は
  **同期完了のたびに走る**ので、選び直す作りにすると読んでいる途中で所見が入れ替わる。
- **一度出した所見は翌日以降しばらく重みが下がる**（1日以内 −40／3日以内 −15）＝
  **日替わりはここで担保する**。乱数にすると同じ日に開き直すたびに並びが変わる。
- **明るい所見と指摘を必ず混ぜる**（2026-09-12 ユーザーの選択「励ましも混ぜる」）。
  上位3件が片側に偏ったら最下位を反対側の最良候補と入れ替える。
  ⚠️ **候補が片側だけの日はそのまま**——無い所見を作らない。
- ⚠️ **`mec_hub_notes_v1` を同期対象に足さないこと**（端末ごとに見せる所見が違ってよい）。
  `test_hub_notes.js` が `progress.js` に現れないことを見張る。

### ⚠️ 描画の約束

- **枠は常設**。所見0件の日も `.note-empty` で中身だけ差し替える
  （「今日はデータが無いから消える」セクションを作らない＝統計ページと同じ規約）。
- **中身が同じなら `innerHTML` を書き換えない**（`_noteLastHtml`）。同期のたびに入場アニメが
  走り直す。
- **横方向に固定寸法を持たせない**。旧レーダーが文字切れしていたのは 160px の箱に枠外へ
  はみ出すラベルを置いていたから。`.note-item` は `flex-wrap` で折り返す
  （実測: 幅 280 / 320 / 358px で切れ・はみ出しゼロ）。
- 動くのは `.note-item` の入場だけで、reduced-motion で止まる（`infinite` は1つも置かない）。

## 🕸 実力の輪郭（2026-09-12・8軸レーダー）

ハブのヒーロー下段、「今日の所見」の左。**8つの科目群 × 自分と全国の二重ポリゴン**。
実装は `index.html` のみ（`_radarStats` / `_radarSnapshot` / `_radarPt` / `_radarGapCls` /
`_renderHubRadar` / `_radarReadout` / `_radarPick`）。
テスト: `node _work/test_hub_radar.js`（20件・実ソースを vm で回す）。

⚠️ **旧「臨床スキルプロファイル」を戻さないこと**（→ 📋 今日の所見の節）。あれは6軸すべてが
固定係数で、科目のデータを1ビットも読んでいなかった。ここはその席を実データで作り直したもので、
**所見（日々変わる＝行動）とレーダー（ゆっくり変わる＝実力）で層を分けている**。

### ⚠️⚠️ 半径に「自分の生の正答率」だけを使わない

弱点カルテで確立した理屈がそのまま効く——**全国的にも難しい領域は、生の％で塗ると
「問題の難しさ」が「弱点」に見える**。実線＝自分／点線＝**同じ問題の**全国正答率の二重ポリゴンで
描き、**面積の差**を実力として読ませる。

- ⚠️ 全国正答率は **uid 単位**で引く（`window.MEC_RATE`）。**科目平均で代用しないこと**——
  stats.html が廃止した「📐本番との差」と同じ誤り（引き算の左右で分母が違う）になる。
- 集計の式は `buildKarte` / `karteGap` と同じ。**ハブのレーダーと統計のカルテが必ず同じ数字を言う**。
- ⚠️ 配色は **青＝上回り／琥珀＝下回り**（カルテと同じ言語・色覚の都合で赤緑を使わない）。
  ±5pt 以内は無彩色。**所見フィードの緑とは別の軸なので混ぜない**。
  ⚠️ **頂点の色をテーマに振らないこと**（意味を持つ色）。テーマに振ってよいのは
  `.radar-box` の地・`.rd-title`・`.rd-self` / `.rd-ghost` の線・`.rd-lbl-sel`・`.rd-go` だけ。

### ⚠️⚠️ 未測定の軸を 0 に落とさない

受験が `RADAR_MIN_N`(=20) 問に満たない軸は、**自分と全国の両方を同じ中立値
（測定済みの軸の全国平均 `natMid`）へ置き**、頂点を白抜き・ラベルを薄くして
「測っていない」と読ませる。0 に落とすとその軸だけポリゴンが中心へ食い込み、
**「実力ゼロ」に見える**（測っていないことと、できないことは違う）。実装時に実際に踏んだ。

### 軸（`RADAR_AXES`）

| 軸 | 科目 | 問題数 | 全国正答率あり |
|---|---|---|---|
| 循環・呼吸 | circ, resp | 1,083 | 97% |
| 消化・肝胆 | dige, hbp | 919 | 88% |
| 腎・内分泌 | jinzo_d, endo | 857 | 88% |
| 血液・免疫 | hema, imma, kansen | 981 | 90% |
| 神経・精神 | neur, psy | 850 | 92% |
| 小児・産婦 | peds, obg | 1,058 | 94% |
| 外科系 | ortho, oph, ent, uro, derm, anes, rad | 1,204 | 94% |
| 救急・公衆 | emg, tox, ph | 735 | 99% |

- ⚠️ **科目を足したら `RADAR_AXES` にも入れること。** どの軸にも入らない科目は黙って落ちる。
  テストが `MM_SUBJECTS`（＝`gamify.js` の SUBJECTS）と突き合わせて取りこぼしを検出する。
- ⚠️ **過去問・実力試験Ⅰ・模試・必修講座・自作・暗記メモは軸に載らない**——科目が付かないため
  （過去問1,825問には科目バッジが1つも無い）。**対象外の件数は必ず脚に出すこと**（`#radarFoot`）。
- ⚠️ チャート上のラベルは**6文字以内**（正式名は読み取り行に出す）。長い名前を盤面に置くと切れる。
- ⚠️ **viewBox にラベルぶんの余白を取ること**。旧レーダーが切れていたのは 160×160 の枠に
  枠外へ出るラベル（右 x=142/anchor=start・左 x=18/anchor=end）を置いていたから。
  テストが `CX ± (R + 40)` が viewBox に収まることを検査する。
  ⚠️ SVG の `font-size` は CSS px で**盤面の縮尺に連動しない**ので、箱を狭めるとラベルの占有が
  相対的に増える。実測: 幅 280px でも左右に 17px 以上の余白が残る。

### ⚠️ ゴースト（30日前の自分）は月次スナップショット

`myrate_v1` は累積で履歴を持たないので、`mec_radar_snap_v1` に
`{'YYYY-MM-DD': {軸id: [受験数, 正解数, 全国正答率の加重和]}}` を**1日1回だけ**書く。

- ⚠️ **今日ぶんを書く前にゴーストを選ぶこと**（初日に自分自身を重ねない）。
- `RADAR_GHOST_DAYS`(=30) ± `RADAR_GHOST_TOL`(=12) 日の範囲で最も近い日を採る。近すぎる履歴は使わない。
- 100日より古いスナップショットは捨てる。受験が `RADAR_MIN_N` に満たない軸は過去ぶんも出さない。
- ⚠️ **同期対象にしないこと**（端末ごとに履歴が違ってよい。同期させるなら日付キーの
  union マージを `progress.js` に足す必要がある）。テストが `progress.js` に現れないことを見張る。

### ⚠️ 中身が同じなら描き直さない

`renderHero()` は**同期完了のたびに走る**ので、`_radarLastHtml` で同じなら `innerHTML` を
差し替えない（選択中の軸の見た目が飛ぶ）。軸をタップしたときも**集計を数え直さない**
（`_radarLast` を使い回す——`_radarSnapshot` は書き込みも行うため）。

## 全国正答率の索引（`rate_index.js`・2026-09-12 に作り直し）

**派生物。`node _work/build_natrate_index.js` で作り直す**（`--check` で現物との一致だけ見る）。
材料は `questions_*.json` の `rate`（無ければ `rate_text`）と `国家試験過去問/**/*.html` の `data-rate`。

| | 旧（2026-07-09） | 新 |
|---|---|---|
| 件数 | 4,939（**コア12科目だけ**） | 9,133（全科目＋過去問1,816） |
| サイズ | 98KB | 32KB（gzip 12KB） |

- ⚠️ **公開する形は `window.MEC_RATE`（uid → 数値）のまま変えないこと。** 章ごとの数値列に
  畳んであり読み込み時に展開する（平坦なまま書くと約145KB）。消費者は stats.html の
  弱点カルテ（`stats.html:1892`）とハブの実力レーダー。
- ⚠️ **`questions_*.json` を更新したら `build_qmeta.py` と一緒にここも流し直すこと。**
  `test_hub_radar.js` が `--check` で食い違いを検出する。
- 直った既存バグ: stats.html の `natRateOf` は「RATE を先に引き、無ければ `qmeta.r`」なので、
  **作り直した循環器・呼吸器・肝胆膵は古い正答率で全国比が計算されていた**。マイナー講座・
  公衆衛生・必修講座は RATE に1問も無かった。
- ⚠️ 値は整数へ丸める（過去問だけ `data-rate` が小数）。読むのはカルテとレーダーで、
  どちらも pt 単位の差しか見ない。**study.html の難易度フィルタ（60/80%）は
  `questions_*.json` の `rate` を直接読んでいてこの索引を読まない**ので影響しない。

### ⚠️ 感染症の `rate` が全問 −1 だった（2026-09-12 に修正）

正答率は `rate_text`（"63%"）にだけ入っていて数値フィールドが −1 のまま＝**356問中348問**。
`rate_cls`（cm/ch/cl）は正しかったので画面上は難易度の色が付いており気づきにくかった。
`rate` は難易度フィルタ・`qmeta.r`・この索引が読むので、**感染症が全国比の分析から
構造的に抜けていた**。修復は `node _work/fix_kansen_rate_field.js`（冪等・`--dry-run` あり）。
⚠️ `questions_kansen.json` は `JSON.stringify(j, null, 1)` + LF・**末尾に改行なし**で往復一致する
（ファイルごとに整形が違うので他科目へ流用するときは必ず確認すること）。
...
- ⚠️⚠️ **カルテの数字は「生の正答率」ではなく「同じ問題の全国正答率との差（pt）」**（2026-09-11〜）。
  「検査」「治療」は全国的にも難しい問題が多いので、生の％で塗ると**問題の難しさを弱点と誤読させる**。
  `buildKarte` は解いた問題ごとに全国正答率を足し込み（`nt`/`nc`/`ne`＝受験回数・正解数・期待正解数）、
  `karteGap` が `自分 − 全国` を返す。**科目単位の平均どうしを引かないこと**（廃止した「📐本番との差」の誤り）。
- 全国正答率は **`RATE`（rate_index.js）を先に引き、無ければ `qmeta.r`** に落とす（`natRateOf`）。
  ⚠️ `rate_index.js` は 2026-07-09 から再生成されておらず、**マイナー講座・公衆衛生の問題を1問も持たない**
  （循環器・呼吸器・肝胆膵の作り直した59問も古い値）。`qmeta.r` は現行の questions_*.json と一致する。
- 上段の **「🚨 優先して潰す弱点」TOP5 は失点（全国並みの期待正解数 − 実際の正解数）の多い順**。
  率だけで並べると「3回中0回」の小さいマスが上に来るため。対象は受験 `KARTE_TOP_MIN_N`(=8) 回以上・
  全国を `KARTE_TOP_GAP`(=5) pt 以上下回るマスだけ。行列の同じマスにも順位の丸印を打つ。
- 配色は **下回り＝暖色4段（`k-d1`〜`k-d4`）／上回り＝青2段（`k-u1`〜`k-u2`）／±5pt 以内＝無彩色**
  （`karteLv`）。赤緑は使わない。全マスに差と自分の％を印字し、タップで読み取り行（`#kReadout`）に全部出す。
- 「全国の正答率帯ごとの自分の正答率」は帯ごとに**全国平均の縦線と、届かなかった幅の斜線**を1本の目盛りに重ねる。
  ⚠️ 旧実装は塗りを `<span>`（インライン）で書いていて `width` が効かず、**4本とも空のバー**だった。
- ⚠️ カルテのマスから study.html へは `?sid=` だけで飛ぶ。**`filter=weak` は study.html の `VALID_FILTERS` に無く
  「全問」へ落ちる**（弱点リスト・サマリーの「🚨苦手問題を解く」「🚩赤旗問題を解く」は今もこの壊れたリンクを持つ）。
...
### 成績カルテ（`mock_karte.html`・2026-09-09〜）

ハブのタイル **🩺 模試 成績カルテ**（`HUB_TILES` の `karte`）と mock.html の結果画面から開く。
`MecMock.score(examId)` が返す `rows` / `blocks` を集計して並べるだけのページ。

- ⚠️ **式を1つも持たないこと。** 正誤・得点・重みは全部 `mock.js` に聞く（上の「正誤を保存しないこと」と同じ理由）。
  このページが持ってよいのは**集計と並べ替えと言葉**だけ。小さい n の順位を信用しないための
...
- ⚠️ **m121s の `_rates.js` は派生物（2026-09-10〜）——直接編集しないこと。**
  材料は手で転記した2つ＝`_work/m121s_seiseki_2026-09-14.tsv`（個人成績表PDF「正誤一覧」400問の
  正答・解答・全国正答率）と `_work/m121s_report_2026-09-14.json`（個人成績表の総合・ブロック別・
  系統別・高正答率問題＋成績判定シート）で、`node _work/build_mock_m121s_rates.js` が書き出す
  （2026-09-10 版の2ファイルは旧集計の記録として残してあり、読まない）。書き出す前に ①正答列＝解答表（400問）
  ②解答列を `mock.js` で採点＝成績表の9区分の得点 ③禁の印＝禁忌肢選択数
  ④同じ採点のブロック別・系統別（必修ブロックを除く）＝成績表のその欄
  ⑤全国正答率≥60%（必修は≥80%）の問題だけの得点＝「正答率が高い問題の成績」（全国正答率の列の転記ずれを検出）
  を突き合わせ、1つでも外れたら書かない（行ずれ・取り落としは①で必ず落ちる）。
  **集計が更新された成績表が届いたら、日付の新しい TSV/JSON を作ってスクリプトの2つのパス定数を差し替える。**
  ⚠️ `judge`（成績判定シート）のランクは**学内の偏差値による相対評価**で、`grades.main`（全国の判定）とは
  物差しが違う（2026-09-14 時点で全国 A・学内 B）。カルテは両方を並べ、別物だと明示している。
...
- **今後の学習方針（`secPlan`・2026-09-10〜）** は「合否に効く順」＝禁忌肢 → 必修 → 取りこぼし（全国80%以上の誤答）
  → 全国との差が小さい系統（全体の差の 3/4 未満・n≥10・最大2つ）→ 深追いしないこと（全国60%未満の誤答・満点の分野）。
  ⚠️ **今回の数字も分野名も直書きしないこと**——全部このページの集計から毎回作る＝解答を直すと方針も書き換わる。
  リンク先の科目は `STUDY_SID`（模試の分野 → study.html の sid）が持つ。**study.html に科目を足したら見直すこと**。
  ⚠️ 「満点の分野」は系統と組にして数える（小児科の下にも「呼吸器」「神経」がある）。
...
## 模試の解説（2026-09-09〜・questions_m121s.json）

`_work/build_mock_m121s_json.py` が解説書PDFから**全400問**を起こす。設計と経緯の正本は
`_work/夏メック模試_引き継ぎ.md` §6。テスト: `node _work/test_mock_questions.js`（24件）。

### ⚠️ questions_m121s.json は純粋な派生物——直接編集しないこと

手書きの解説（誤答65問を循環器品質で書き下ろすぶん）は **`_work/mock_m121s_overrides.json`**
に uid をキーにして置き、**生成の最後にマージする**（2026-09-08 ユーザー裁定）。
直接書き込むと、PDFの読み取りを直して再生成した瞬間に消える。

```bash
python _work/build_mock_m121s_json.py            # 書き出す（手書きを重ねてから検算）
python _work/build_mock_m121s_json.py --block A  # A問題だけ検算（書き出さない）
python _work/build_mock_m121s_json.py --figs     # 画像診断の図を 夏メック模試/images/ex/ へ
python _work/build_mock_m121s_json.py --check    # 現物と一致するかだけ見る
```

### ⚠️ uid の対応は `MecMock.studyUid()` が唯一の正本

採点ツールの uid（`m121s_A_q17`）と解説の uid（`m121s_ch01_q17`）は**別物**。
対応は「ブロックの count を積む」規則1本で、`mock.js` の `studyUid()` と生成器の両方が
同じ規則を持ち、テストが一致を見張る。⚠️ **章頭の通し番号（A:1 B:76 C:126 …）を
どこかに書き写さないこと**——ブロックの問題数から決まる値なので、数字で持つと腐る。

### ⚠️⚠️ MEC の ○/× は「正解の印」ではない

選択肢考察の ○/× は**その記述が正しいかどうか**であって「これが正解か」ではない。
否定形の設問（「〜でないのはどれか」）では **× の肢が正解**で、実測**68問**がそうだった。
- `ans_sub`（✅ の直下）は **正解の肢**の考察を引く。○ の行ではない。
- 選択肢考察の表で緑（`kw3`）に光らせるのも**正解の肢**。紙面の ○/×/□ の印は
  そのまま先頭に残す（印は紙面の事実、緑は正解という別の軸）。

### ⚠️ 禁忌肢はカードの見出しに出さない

禁忌肢17問に専用バッジは付けない。解説を開く前に「踏んではいけない肢がある」と分かると
それ自体がヒントになる（`mock.js` の「採点前は禁忌肢問題であることを出さない」と同じ理由）。
紙面の `□` 印は**選択肢考察の中＝正解を見た後の場所**にだけ残す。テストが見張る。

### バッジ

`bb`（新設・桃）＝紙面の問題番号「A問題 17」。表示番号 `Q.n` は科目内の通し番号なので、
マークシートと突き合わせる番号をここで別に出す。ほかは既存のものを使い回す——
`bh` 必修（B・E の100問）／`bip` 一般・`brn` 臨床／`bsub` 科目（`cat2`）／`bm` N択／
`bk` 計算（桁入力3問）／`bi` 📷画像。⚠️ クラス名は `bg`（土台）・`br`（連問）と衝突させない。

### ⚠️ 解説の図（`夏メック模試/images/ex/`・140枚）は設問の図と別物

「画像診断」ブロックの**注釈付きの図**（矢印と標識入り）で、答えそのものなので
**解説ブロックの中**に出す。設問の図（`q.imgs`・素の写真138枚）は選択肢より**上**に出る。
- ⚠️ **`q.imgs` に入れないこと**（選択肢より上＝答えが先に見える）。
- ⚠️ `class` は `qimg exfig`。`qimg` はライトボックスが掴む名前なので外さない。
  一方で**行を `.qimg-row` にしないこと**——「🖼️画像だけ」フィルタの `hasImg` が
  `.qimg-row .qimg` を数えており、解説に図があるだけで画像問題に化ける。
- ⚠️ 説明文（「写真A　…」）と図は**紙面と同じ交互の並び**で入れる。文を先にまとめると
  どの説明がどの絵か読めない。
- ⚠️ 図に添えられた注記（矢印の先の「顆粒状隆起性病変」）は**図の一部**。本文から外して
  clip へ畳む。残すと語が説明文に紛れ、絵からは切れて矢印だけが残る（両方で壊れる）。
- ⚠️⚠️ 注記を畳む相手は**同じページの、一番近い図**。ページを見ないと別ページの図へ
  吸い込まれて消え（F70）、近さを見ないと隣の図の説明文まで巻き込む（A38）。

### ⚠️ 選択肢が表の2問（A65・D46）は生成器の定数が正本

`TABLE_CHOICES`。表のまま置くと試験モードの肢シャッフルで対応が崩れるので、
**列見出しを各肢に埋め込み**、単位は設問文の末尾に注記する（CLAUDE.md
「表・図の選択肢の復元」と同じ流儀）。⚠️ 選択肢は `esc()` で描かれるので**HTMLタグは
使えない**——上付き下付きは Unicode（`₂ ₃ ⁻ ⁺`）で書く。

### この模試には正答率が無い

解説書の全772ページを走査して「正答率」は0件。`rate: -1` / `rate_text: ''` なので、
難易度フィルタ（難問/標準/易問）と `_isHardCard` には掛からず「正答率なし」に入る。
これは欠落ではなく模試の性質——**推測値を入れないこと**。
...
テスト: `node _work/test_stats_sections.js`（28件）・`node _work/test_karte.js`（19件）。
...
### 演出（2026-09-11 に「しっかり動かす」で再導入）

`f9c351a`（2026-08-23）で一度ゼロになった演出を、**ユーザーが「しっかり動かす」を選んで**戻した。
過去2回「ページが落ち着かない」で撤去された経緯は伝えたうえでの判断＝**タブを開くたびに入場が走る**
（旧条件③「セッション内で1回だけ」は意図的に外した）。旧 `armReveal` / `.rv-on` は復活させていない。

| 仕組み | 実装 |
|---|---|
| 許可の印 | `html.fx-on`。`<head>` の1行と本体の `fxApply()` が **reduced-motion でないときだけ**付ける。演出の CSS は**全部この下**に書く（テストが見張る） |
| セクションの入場 | `.tab-pane.active > .sec` に `fxSecIn`。`display:none` から戻るとアニメは頭からやり直しになるので JS 不要。順番は `--si` |
| 見えたら動く塊 | `.fxb` に `fxArm()` が `.fx-go` を付け直す。**画面内のものはその場で同期的に**（IO は次フレームなので一度静止画が見えてちらつく）、下にあるものは IntersectionObserver（`rootMargin` 下20%）で |
| 数え上げ | `.fx-go` の中の `[data-cu]` を `countUp()`。本文は**確定値で描いてから**一時的に書き換え、`setTimeout(finish, dur+400)` で必ず戻す。途中で描き直されたらそちらを尊重する |
| タブの地 | `.tab-ink` が滑る。タブ切替・コピー完了で `MecFX.burst`（`MecFX.setEnabled` を reduced-motion に連動） |

- ⚠️ **移動・拡大は `translate` / `scale` で書くこと**。`.pace-bar-fill` は `transform:scaleX()` を
  インラインで持つので、`transform` のアニメを当てると値が消える（study 側の Phase 7 と同じ罠）。
- ⚠️ **フォーカス切替のような「見た目だけの操作」で描き直さないこと**（`🎯 下回るマスだけ` はクラスの
  付け替えだけ）。描き直すと入場演出がまた全部走る。
- ⚠️ 常時動くもの（`infinite`）は **弱点TOPのマスの輪・斜線の流れ・国試リングの光・今日のセル・
  要復習バッジ・主ボタンの光沢・期限切れの棒** に限っている（実測で落ち着いた状態の running が15本）。
  **増やすなら小さい要素だけ**にし、`.qc` のようにたくさん並ぶものへ付けないこと。
- ⚠️ headless Chrome の `--virtual-time-budget` は infinite アニメがあると終わらない。
  スクリーンショットは DevTools Protocol で撮る（2026-09-11 に踏んだ）。

以下は過去の撤去で得た、**動きを足すときに必ず添える条件**（今も有効）。
- ⚠️ **隠すのは JS が付けたクラスの中でだけ**（旧 `.rv-on .rvs{opacity:0}` の作り）。
  CSS 側だけで `.rvs{opacity:0}` にすると、JS が落ちた日に**ページが丸ごと真っ白**になる。
  `test_stats_sections.js` が「CSS だけで本文を隠していない」を見張っている。
...
- ⚠️ **rAF で数字を動かすなら `setTimeout(finish, dur+400)` の落とし所を必ず添える**
  ...
  テストは**rAF を使っているのに落とし所が無いときだけ**落ちる。
- ⚠️ **`prefers-reduced-motion` の止めは2重**——`fx-on` を付けない（本体）＋ `@media` で全停止（保険）。
  動きを足すなら**必ず `html.fx-on` の下に**書くこと。
- 「今日」のセルは `.cal-cell.is-today{outline:1.5px solid var(--yl)}` の**静的な印**に加えて、
  2026-09-11 から呼吸（`fxToday`）がある（12週＝84セルの中で今日の位置が毎日ずれるので印は要る）。
  ⚠️ **`outline` は消さないこと**——reduced-motion で動きが止まっても位置が
...
正本は `study_exam.js` 末尾の「正解・誤答の演出」節（`_rf*`）＋ `study.css` 末尾。
⚠️ **過去問ビューア（`chapter_exam.js`）は旧演出のまま**（意図的に乖離させている方針は従来どおり）。
下の「試験モードの演出エフェクト仕様」「試験経路の演出」などで `_showStreakEffect`・全画面フラッシュ・特大×n・
トースト（`#examStreakToast`）・`_afterWrongFx`・`_wrongDamageFx`・バレットタイムを語っている箇所は、
**study 側では削除済み**＝過去問ビューア側（`ce*`）の記述として読むこと。

- **正解**：「正解の肢から、順番に」出す（0ms 肢の縁を光が1周 `_rfSweep` → 80ms 光が数字へ走る → 200ms UIテーマ固有演出を
  **肢の位置で**（`_spawnStreakParticles(tier, at)`） → 240ms 連続数 `#examRfStreak`）。
  全画面フラッシュ・画面の揺れ・浮遊コンボ・外周パルス・散らばった位置の粒子は出さない。暗転は**段が上がった瞬間だけ**。
  量は旧演出と同程度（ユーザー判断）——**抑えすぎないこと**（一度「抑え過ぎ」と指摘されている）。
  ⚠️ 連続数は専用要素 `#examRfStreak`（書体・枠はUIテーマ8種ぶん。絵文字と「！」を使わない）。
  ⚠️ 正解の合流点は `_rfCorrectFx` の1つ（`revealAnswer`・`_revealCalcAnswer` の2経路が呼ぶ）。`_afterCorrectFx` はその中から呼ぶ。
- **誤答＝答えを見せずに選び直させる**（ユーザー判断）。1回目の誤答で採点・記録を済ませ（不正解）、
  カードは `.exam-retry`（`exam-revealed` を持たない）。外した肢は `.exam-out`＋×。何回でも選び直せて、
  「▶ 答えを見る」（＝`revealAnswer` → `_rfGiveUp`）で打ち切れる。計算問題は入力し直して再確定。
  赤いフラッシュ・揺れ・被弾アニメは出さない（連続の崩落・心電図の平坦化・「前にも同じ肢」は残す）。
  ⚠️⚠️ **2回目以降のクリックを採点経路に通さないこと**（myrate・SRS・attempts・今日の誤答は必ず1回目の結果）。
     誤答の採点の口は `_rfScoreWrong` の1つ（`_rfWrongPick`・`_rfCalcSubmit`・`_rfRevealUnanswered` が呼ぶ）。
  ⚠️ **肢を選ばずに「▶ 解答を見る」／Enter で開く＝不正解として記録**（`_rfRevealUnanswered`）。
     2026-09-24 まではこの操作が正解として記録されていた（`revealAnswer` が選択の無いカードで `.ch2.ok` を拾っていた）。
  ⚠️ `_saveExamResume` は `.exam-retry` を開封済み（不正解）として保存する＝再開すると答えが開いた状態で戻る。
  ⚠️ 数字キーは `.click()` に流して同じ経路を通す（キーボード側に分岐を複製しない）。
  ⚠️ 選択肢に水平の線を作らないこと（× は右端の文字だけ。下線部はこの教材で意味を持つ）。
...
- **Liquid のテーマ固有演出（2026-09-25 に置き換え）**：デモ（`liquid_fx_lab`）の案G「油膜の虹彩」＋案J「メッシュグラデーション」を採用。
  正解の肢の**文字の裏**（`.lq-layer`・z-index:-1）で色の塊4つが咲いて混ざり、薄膜の虹色が輪になって渡る。TIER3〜はカードの裏にも色が広がり、
  カードの縁が油膜の色で回る（`.lq-ring`）。段が上がった瞬間は虹色の輪がカード全体を渡る。実装は `_lqFluidFx`（study_exam.js）＋ study.css 末尾。
  旧 `liquidBloomRipple`（全画面の閃光・絞り羽根・画面の下からの泡・画面全体の粉・火花）は正解演出から外した（結果画面では今も使う）。
  ⚠️ **0ms で出す**（他テーマは 200ms）。正解の 300〜400ms 後に次のカードへ自動スクロールするので、待つと肢が画面から去ってから咲く。
  ⚠️ WAAPI の加減速は**区間ごと**に掛ける。全体に `MO.out` を掛けると咲く→消えるが最初の1/4に潰れて見えない（実際に踏んだ）。
  ⚠️ 試験モードでは ui_theme.css の旧・正解肢の層（オーラ・波紋・ぷるん）を `body.exam-mode` に閉じて止めている。過去問ビューアは旧のまま。
- **Frost のテーマ固有演出（2026-09-25 に置き換え）**：デモ（`frost_fx_lab`）の案A「六花」＋案B「霜華」＋案F「ダイヤモンドダスト」を採用。
  正解の肢が縁から凍ってタップ位置から溶け（肢の裏）、カードの裏に雪の結晶が線で描き上がる（**段が上がるほど枝が複雑になる**・毎回ちがう形）。
  光の粒が肢の上で舞い降りて瞬く（`.fr-dust`＝文字の上だが小さく疎ら）。TIER3〜は四隅の霜・小さな結晶・カード全体の粒、段が上がった瞬間は縁が一周凍って溶け・大きな結晶・粒が左→右に瞬く。
  実装は `_frFrostFx`（study_exam.js・liquid の `_lqLayer`/`_lqDrop`/`_lqPtr` を共用）。旧 `frostCrystalShatter`（全画面の閃光・約300粒）は正解演出から外した（結果画面では今も使う）。
  ⚠️ 六花は**肢の層とカードの層に同じ位置・角度で2回**描く（肢の地は不透明度.75＝カードの裏だけだと肢に重なる部分が隠れる）。
  ⚠️ カードの canvas は肢のまわり最大 `FR_BAND`(1000px) の帯に限る（解説まで開いた長いカード全面だと数十MBになる）。0ms に出す理由は liquid と同じ。
  ⚠️ 未修正：`MecFX.bubbles(o)` は引数1つなのに結果画面などで `bubbles(cx, cy, {…})` と呼んでおり、色と数の指定が読まれず既定の金色24個が画面の下から出る（Abyss も同じ）。
- **Celestial のテーマ固有演出（2026-09-25 に置き換え）**：デモ（`celestial_fx_lab`）の案C「金環」を**毎回**出し、案A「星座」・案B「惑星直列」・案E「星の軌跡」から**毎回ランダムで1つ**を重ねる（ユーザー指定）。
  金環＝タップ位置の金の太陽に月が重なり、抜ける瞬間にダイヤモンドリング（段でコロナが伸びる・TIER3〜はカードの裏まで・段が上がった瞬間は光条が十字にカードの端まで）。段が上がった瞬間は重ねた1つも大きくなる（本物の星座と名前／大きな太陽系／カード全面の星の軌跡）。
  実装は `_clxCelestialFx`（study_exam.js・`_lqLayer`/`_lqDrop`/`_lqPtr`/`_frCtx`/`_frRun`/`FR_BAND` を共用）。旧 `celestialAstrolabe`（全画面の紫の閃光・約250粒・`diamondSparkle` の二重呼び出し）は正解演出から外した（結果画面では今も使う）。
  ⚠️ 絵は全部**肢の層とカードの層に同じ座標で2回**描く（六花と同じ理由）。月は色で塗らず `destination-out` でその層の太陽を抜く＝どちらの層でも下の地の色が月になる。文字の上には何も描かない。
- **Brass のテーマ固有演出（2026-09-26 に置き換え）**：デモ（`brass_fx_lab`）の案A「歯車列」＋案C「刻印」＋案D「鋳込みの唐草」を採用。
  歯車列＝タップ位置の駆動歯車から左右へ歯車が噛み合って連なり、歯数比どおりに回る（2＋段・最大9枚・TIER3〜はカードまで）。刻印＝肢の右端に「MEC」の楕円の刻印が白熱から真鍮色へ冷え、左に紙面の問題番号（No.0214）。唐草＝タップ位置から斜め4方向へ溶けた真鍮の蔓が伸びて渦を巻く。段が上がった瞬間は大きな歯車列がカードを横切り、中央にローマ数字のメダルが打刻され、四隅から唐草の飾り金具が伸びて斜めの光沢が走る。
  実装は `_brsBrassFx`（study_exam.js・`_lqLayer`/`_lqDrop`/`_lqPtr`/`_frCtx`/`_frRun`/`FR_BAND` を共用）。旧 `brassClockworkBurst`（全画面の金の閃光・飛び散る歯車・画面全体の蒸気と粉・火花の二重呼び出し）は正解演出から外した（結果画面では今も使う）。
  ⚠️ 描くのは**カードの層1枚だけ**（brass の肢の地は半透明＝frost・celestial のように肢の層へ二重に描くと肢の中だけ濃くなる）。文字の上に出るのは火花（`MecFX.sparks` 数粒）だけで、位置は発火の瞬間にカードから測り直す。
  ⚠️ 大きさの基準は肢の高さ（最低 `BRS_MIN_H`＝44px）。実物の肢は1行だと約28pxで、そのまま測ると TIER1〜2 が豆粒になる。
  ⚠️ 唐草は肢の中を横に走らせないこと（ラボの初版で主蔓が肢の文字の取り消し線・下線に見えた）。斜め4方向の十字の飾りにしてある。
...
| 設定キー | 既定 | 中身 |
|---|---|---|
| `mec_correct_sound_v1` | 一覧の先頭（`custom`＝正解音） | `sounds/正解音/` にあるもの |
| `mec_select_sound_v1` | 一覧の先頭（`mp3`＝選択） | `sounds/選択音/` にあるもの |
| `mec_result_sound_v1` | 一覧の先頭（`fanfare`＝勝利のファンファーレ） | `sounds/結果画面/` にあるもの |
| `mec_boot_sound_v1` | — | **もう読んでいない**（起動音は常に鳴る・下記） |

- ⚠️⚠️ **「無音」は 2026-09-10 に全スロットから廃止した（ユーザーの判断）。戻さないこと。**
  設定画面に無音のボタンが無いので、**保存済みの `'off'` を素通しすると二度と音を戻せない**
  ＝`_sndResolve`（study_exam.js）・`_sndResolveHub`（index.html）・`ceSndResolve`（chapter_exam.js）が
  **必ず一覧の先頭＝既定へ落とす**（localStorage は書き換えない）。起動音は保存値を見ずに常に鳴らし、
  ボタンは「🎲 ランダム（タップで試聴）」の1つだけ＝押すたびに抽選し直す試聴になった。
  `test_sounds.js` の [4b] がこの3点を見張る。
  ⚠️ 種類の数を文書に書かないこと（音を足すたびに腐る）。実測は `sounds_index.js`（＝生成物）。
...
- ⚠️ **起動音は設定で選ばせず、試験開始のたびにランダムで1つ鳴る**（2026-08-21〜）。
  ...
  旧値（`'ms'` 等＝ファイルを指していた頃の設定）は一切見ない＝常に鳴らす。
- ⚠️ **起動音の尺はカウントダウンより長くてよい＝鳴らし切る**（2026-08-21 にユーザーが選択）。
  4.39〜6.36秒（現物）に対しカウントダウンは 2.535〜2.745秒なので、明けて
  1問目に入っても音だけ2〜4秒続く。**`_examCountdown` の尺は1msも増やさない**
...
- ⚠️ **キーは localStorage にそのまま入るので改名しないこと**（`custom`＝correct.wav、
  `mp3`＝選択.mp3 は既存ユーザーの設定が刺さっている）。ファイルを移しても**改名しても** key は据え置く
  （2026-09-10 に MHF・キングダムハーツ・ゼルダの伝説がファイル名だけ変わったが key は `mhf`/`kh`/`gomadare` のまま）。
  ⚠️ 逆に**ファイルを消したらその key も一緒に消す**——`test_sounds.js` の [7]「旧キーが生きている」の
  一覧からも外すこと（消した音の key を残すと、実在しないキーを守る空回りの検査になる）。
