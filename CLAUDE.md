...
| `耳鼻咽喉科/` | マイナー講座・耳鼻咽喉科（prefix `ent`・👂・#0F766E）。章別HTML(`ch01_mimi_kihon.html`〜)＋`images/`＋`ent_questions.json`（全8章の章名メタ）。HTMLが`questions_ent.json`のソース＝章ごとの生成器`_work/build_ent_ch{NN}.py`→`_work/build_ent_json.py`で再生成。眼科と同方式。**全8章214問・章頭NO.は 1／21／51／99／102／138／152／172**（章名はPDF表記のまま「耳①：耳の基本」等）。**2026-08-06に第1章「耳①：耳の基本」NO.1-20（画像8問14枚）と第2章「耳②：耳の疾患（中耳）」NO.21-50（30問・画像23問41枚）、第3章「耳③：耳の疾患（内耳・後迷路性・その他）」NO.51-98（48問・画像19問39枚＝**ent最大の章**）を追加**。ch02の軸は急性中耳炎9問・真珠腫性中耳炎7問・滲出性中耳炎5問・耳硬化症3問・慢性穿孔性中耳炎3問で、**鼓膜写真1枚から疾患を決めて治療（保存/鼓膜切開/チューブ/鼓室形成術/アブミ骨手術）へ橋渡しする**筋が繰り返し問われる。ch03の軸はBPPV 9問・Ramsay Hunt 8問・聴神経腫瘍6問・騒音性難聴6問・老人性難聴5問・Ménière病4問・前庭神経炎3問で、**めまいは「持続時間」（秒＝BPPV／分〜時間＝Ménière／日＝前庭神経炎）、難聴は「オージオグラムの型」で疾患を決める**という筋。
⚠️ **ch03は連問が1組ある**（NO.80・81＝109C-28/29）。**症例文は両方のカードに載せる**こと——試験モードはカードを1枚ずつ独立に出すので2問目に症例が無いと解けない（設問文と選択肢は問題ごとに別物にする。生成器では `_STEM_80` 定数を両方の qt に連結している）。
...
⚠️ **図中の①〜⑤／①〜⑦が選択肢そのものになる問題がある**（ch01は NO.10・15・19／ch02は NO.25・34・43・**45**）。PDFを260〜600dpiで拡大して目視で同定すること。**ch02 NO.45(105G-48) は選択肢a〜eが5枚のティンパノグラムの図で、本文に選択肢の文字が1つも無い**——図を読んで各肢の文章を書き起こした（正解 e＝ピークの無い平坦なB型）。
...
⚠️ **採点除外は NO.76（110A-28）の1問**（`bx`バッジ・正解肢0・`rate=None`）。**正答率が無いのは 13・49・50・70・117・150・151・192 の8問**（84B/95A/98/99など旧国試で出典に数字が無いだけ＝採点除外ではない）。
...
| 眼科（マイナー講座） | oph | 8 | 213 |
| 耳鼻咽喉科（マイナー講座） | ent | 3／8 | 98／214（進行中） |
| 実力試験Ⅰ | jitsu1 | 2 | 160 |
...
※ 耳鼻咽喉科は2026-08-06に第1章20問・第2章30問・第3章48問を追加。**全8章214問の予定**（耳20/30/48・鼻口唾3/36/14・咽喉頭20/43）。`study.html` の科目ヘッダは実数（現在98問）を出す。
