...
| `整形外科/` | マイナー講座・整形外科（prefix `ortho`・🦴・#A16207）。章別HTML(`ch01_seikeigeka_kihon.html`〜)＋`images/`＋`ortho_questions.json`（全6章の章名メタ）。HTMLが`questions_ortho.json`のソース＝章ごとの生成器`_work/build_ortho_ch{NN}.py`→`_work/build_ortho_json.py`で再生成。泌尿器科と同方式。**全6章174問の予定・章頭NO.は 1／31／74／107／140／167**（章名はPDF表記のまま「整形外科の基本」「神経の障害」「関節疾患」「骨　折」「その他の疾患」「ロコモティブシンドロームとサルコペニア・フレイル」）。**2026-08-12に第1章「整形外科の基本」NO.1-30（30問・画像10問19枚）を追加**。
...
| 整形外科（マイナー講座） | ortho | 1／6 | 30／174（進行中） |
| 実力試験Ⅰ | jitsu1 | 2 | 160 |
...
※ 整形外科は2026-08-12に第1章「整形外科の基本」30問を追加（全6章174問の予定＝基本30／神経の障害43／関節疾患33／骨折33／その他27／ロコモ8）。`study.html` の科目ヘッダは実数（現在30問）を出す。章を足すたびに実数へ更新すること。
