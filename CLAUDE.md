...
| `耳鼻咽喉科/` | マイナー講座・耳鼻咽喉科（prefix `ent`・👂・#0F766E）。章別HTML(`ch01_mimi_kihon.html`〜)＋`images/`＋`ent_questions.json`（全8章の章名メタ）。HTMLが`questions_ent.json`のソース＝章ごとの生成器`_work/build_ent_ch{NN}.py`→`_work/build_ent_json.py`で再生成。眼科と同方式。**全8章214問・章頭NO.は 1／21／51／99／102／138／152／172**（章名はPDF表記のまま「耳①：耳の基本」等）。**2026-08-06に第1章「耳①：耳の基本」NO.1-20（画像8問14枚）と第2章「耳②：耳の疾患（中耳）」NO.21-50（30問・画像23問41枚）を追加**。ch02の軸は急性中耳炎9問・真珠腫性中耳炎7問・滲出性中耳炎5問・耳硬化症3問・慢性穿孔性中耳炎3問で、**鼓膜写真1枚から疾患を決めて治療（保存/鼓膜切開/チューブ/鼓室形成術/アブミ骨手術）へ橋渡しする**筋が繰り返し問われる。
...
| 眼科（マイナー講座） | oph | 8 | 213 |
| 耳鼻咽喉科（マイナー講座） | ent | 2／8 | 50／214（進行中） |
| 実力試験Ⅰ | jitsu1 | 2 | 160 |
...
※ 耳鼻咽喉科は2026-08-06に第1章20問・第2章30問を追加して着手。**全8章214問の予定**（耳20/30/48・鼻口唾3/36/14・咽喉頭20/43）。`study.html` の科目ヘッダは実数（現在50問）を出す。
