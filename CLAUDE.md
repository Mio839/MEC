...
3. ⚠️ **`pdf_audit.py` が「PDF内のどの図とも一致しない」と出す画像は、ベクター描画で埋め込みraster
   として存在しないもの＝ページ領域を描画して作られたファイル**である。循環器の10件は
   **全部が心電図**だった（Holter・12誘導・モニター心電図・ECG）。消すと **xref 抽出では復元できず、
   矩形を手で決めて描画し直す**という一番高い手戻りになる。
...
1. 巻末解答一覧表の**「疾患名」列**との整合（眼科で確立。心電図・心エコーの読みの裏取りに効く）
2. `pdf_audit.py` の知覚ハッシュ（**ベクター描画は必ず不一致になる**前提で読む）
3. 「〜を示す」等の figure 参照と `imgs` の対応（欠落の受け皿）

- 巻末解答一覧表の**「疾患名」列**は画像の読みの裏取りに使える（眼科で確立した手）。
