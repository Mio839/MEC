...
※ 眼科は2026-07-30に第1・2章76問、2026-07-31に第3章20問・第4章19問を追加（全8章213問の予定）。章が増えるたびに実数へ更新すること。
...
- 科目prefix（全18）: `endo` / `resp` / `circ` / `dige` / `neur` / `hbp` / `jinzo_d` / `hema` / `imma` / `kansen` / `peds` / `obg` / `psy` / `derm` / `oph` / `jitsu1` / `custom` / `memo`
