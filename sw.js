// 2026-08-22t: 学習ハブHeroゲージのテーマ別差別化と段階的進化（Tier 0〜6）の完全実装。
//   - Brass: 歯車列のTier0〜6回転速度連動、立体陰影・ギロシェ逆回転リング・達成時ゴールドオーラを実装。
//   - Aurora: 多層偏光オーロラウェーブ、クリスタルパルス、Tier0〜6速度・色彩連動を実装。
//   - Cyber: 3Dホログラフィックレーダースイープ、タクティカルレティクル、HUDモノスペースカウンターを実装。
//   - Liquid: 有機的フルイドモーフィング、ネオンジェル粘性変形、リップルパルスを実装。
...
const SHELL_VERSION = "2026-08-22t";
