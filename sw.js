// 2026-09-07j: Aurora（オーロラ・グラス）のHeroゲージを極限まで絢爛・神秘的に深化——①立体多面体カッティング・クリスタルフレーム（最外周オクタゴンベゼル・立体ダイヤモンドトラスリブ・分光ノッチ・8頂点クリスタルジュエル）、②重層流麗オーロラカーテン（深層・中層・主光幕・波頭線）＆立ち昇る鉛直極光光芒（Ion Pillars）、③静寂のダイヤモンドダスト＆四芒星スターダスト（16点）、④極光クロノス・ジュエル（四芒星フレア・光彩ハロー）＆360度プリズムアーク、⑤極光分光波長HUD・テレメトリー（391.4nm〜636.4nmおよび発光状態表示）、⑥オーバードライブ光冠放射レイを実装。
//   シェルのみの変更なので CACHE は据え置き＝SHELL_VERSION だけ bump（2026-09-07i → 2026-09-07j）。
...
//       シェルのみの変更なので CACHE は据え置き＝SHELL_VERSION だけ bump。
const CACHE = "mec-v373";
// シェル更新トリガ: この文字列を変えると sw.js のバイトが変わり SW 更新が走る。CACHE 名は
// 据え置きなので CARDS(問題JSON 約15MB)は再DLされない。install が cache:'reload' でシェルだけ
// 最新取得して上書きするため、シェル(html/css/js)を変えたらここを日付+連番で bump すれば確実に届く。
// （questions_*.json を変えた時だけ CACHE 自体を bump ＝全再DL）
const SHELL_VERSION = "2026-09-07j";
